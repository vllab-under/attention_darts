...
class _FPN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        # Bottom-up
        c1 = self.RCNN_layer0(im_data) # c1 [N, 64, 150, 157]
        c2 = self.RCNN_layer1(c1) # c2 [N, 256, 150, 157]
        c3 = self.RCNN_layer2(c2) # c3 [N, 512, 75, 79]
        c4 = self.RCNN_layer3(c3) # c4 [N, 1024, 38, 40]
        c5 = self.RCNN_layer4(c4) # c5 [N, 2048, 19, 20]
        # Top-down
        p5 = self.RCNN_toplayer(c5) # P5 [N, 256, 19, 20]
        p4 = self._upsample_add(p5, self.RCNN_latlayer1(c4))
        p4 = self.RCNN_smooth1(p4) # P4 [N, 256, 38, 40]
        p3 = self._upsample_add(p4, self.RCNN_latlayer2(c3))
        p3 = self.RCNN_smooth2(p3) # P3 [N, 256, 75, 79]
        p2 = self._upsample_add(p3, self.RCNN_latlayer3(c2))
        p2 = self.RCNN_smooth3(p2) # P2 [N, 256, 150, 157]
        
        p6 = self.maxpool2d(p5) # P6 [N, 56, 19, 20]
